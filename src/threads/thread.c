               ...
               thread_func *function, void *aux) 
{
  ...
  /* Add to run queue. */
  thread_unblock (t);
  //yield_if_curr_is_not_max_priority();
  return tid;
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  ///ASSERT(list_empty(&t->donated_list));///for debugging
  ///
}
...
next_thread_to_run (void) 
{
  ...
  if (list_empty (&ready_list))
    ...
    highest = t1;
    while(&t1->elem != list_back(&ready_list)){
...
void
yield_if_curr_is_not_max_priority (void)
{
  struct thread *curr;
  struct thread *t1;
  struct thread *t2;
